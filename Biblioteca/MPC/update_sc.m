...
function [sc,lesp,lesalpha]=update_sc(sc_past,param,Niter, dados)
coder.allowpcode('plain');
sc=sc_past;[Jcurrent,gcurrent]=BBS(sc.p,param, dados);
lesp=zeros(Niter,1);lesalpha=lesp;
for i=1:Niter
    lesp(i)=sc.p;lesalpha(i)=sc.alpha;
    sc=compute_parabola(sc,param, dados);
    if (sc.qgmax<=0)
        pcand=sc.pJmin;[Jcand,gcand]=BBS(pcand,param, dados);
        Cond=(((Jcand<Jcurrent)&&(gcand<=0))||...
                ((Jcand<=Jcurrent)&&(gcand<0)));
    elseif (sc.qgmin>0)
        pcand=sc.pgmin;[Jcand,gcand]=BBS(pcand,param, dados);
        Cond=(gcand<gcurrent);
    else
        sc=compute_Zg(sc);
        if (sc.ngz==1)
               ...
               [Jcand,gcand]=BBS(pcand,param, dados);
        else
