...
struct Matrix {
    ...
    std::vector<double> d;
    Matrix(): r(0), c(0) {}
    Matrix(size_t r_, size_t c_, double v=0.0): r(r_), c(c_), d(r_*c_, v) {}
    void resize(size_t r_, size_t c_, double v=0.0){ r=r_; c=c_; d.assign(r*c, v); }
...
Matrix mul(const Matrix& A, const Matrix& B){ assert(A.c==B.r); Matrix C(A.r,B.c,0.0); for(size_t i=0;i<A.r;i++) for(size_t k=0;k<A.c;k++){ double aik=A(i,k); for(size_t j=0;j<B.c;j++) C(i,j)+=aik * B(k,j); } return C; }

Matrix smul(double s, const Matrix& A){ Matrix C(A.r,A.c); for(size_t i=0;i<A.r*A.c;i++) C.d[i]=s*A.d[i]; return C; }
...
public:
    ...
    Matrix umax, umin; // assumed column vectors (nu x 1)

    // computed matrices (outputs)
    Matrix H, F1, F2, F3;
    Matrix Aineq, G1, G2, G3;
    std::vector<double> utildemax, utildemin;
    ...
    void compute_MPC_Matrices(){
        ...
        // build utildemax/utildemin (stacked umax/umin over horizon)
...
private:
    ...
    void compute_constraints_matrices(){
        ...
        std::vector<double> interPinuN(N*nu*N*nu, 0.0); // large zero matrix flattened (but we only slice)
        Matrix inter_Psi_i = B;
        Matrix Phi_i = A;

        Matrix Aineq_1(0,0), Aineq_2(0,0);
        Matrix G1_1(0, n);
        std::vector<double> G3_11_v, G3_12_v, G3_21_v, G3_22_v;

        // We'll accumulate rows into std::vector< vector<double> > and convert at end
        std::vector<std::vector<double>> rows_Aineq1;
        std::vector<std::vector<double>> rows_Aineq2;
        std::vector<std::vector<double>> rows_G1_1;
        ...
        std::vector<double> G3_1_v; G3_1_v.insert(G3_1_v.end(), G3_11_v.begin(), G3_11_v.end()); G3_1_v.insert(G3_1_v.end(), G3_12_v.begin(), G3_12_v.end());
        std::vector<double> G3_2_v; G3_2_v.insert(G3_2_v.end(), G3_21_v.begin(), G3_21_v.end()); G3_2_v.insert(G3_2_v.end(), G3_22_v.begin(), G3_22_v.end());
...
int main(){
    ...
    // LIMITES
    // =========================
    double pos_limite = 20.0/100.0;
    double ang_limite = 12.0 * (M_PI/180.0);
    double vel_limite = 45.0/100.0;
    double comando_limite = 12.0;
