...
*/
	...
	for( iter=nWSRperformed; iter<nWSR; ++iter )
	{
		...
		{
			if ( ( tau <= QP_EPS ) && ( options.enableRamping == BT_TRUE ) )
				performRamping( );
			else
...
returnValue QProblem::backsolveT( const real_t* const b, BooleanType transposed, real_t* const a ) const
{
	...
	if ( transposed == BT_FALSE )
	{
		...
		for( i=0; i<nT; ++i )
		{
			...
			if ( getAbs( TT(i,sizeT-1-i) ) > QP_EPS )
	...
	else
	{
		...
		for( i=0; i<nT; ++i )
		{
			...
			if ( getAbs( TT(nT-1-i,tcol+i) ) > QP_EPS )
											...
											)
{
	...
	for ( i=0; i<nAC; ++i )
	{
		...
		if ( ( getAbs( delta_lbA[ii] ) > QP_EPS ) || ( getAbs( delta_ubA[ii] ) > QP_EPS ) )
		{
