...
returnValue Options::setToDefault( )
{
	...
	terminationTolerance          =  1.0e2 * QP_EPS;
	boundTolerance                =  1.0e2 * QP_EPS;
	#else
	terminationTolerance          =  5.0e6 * QP_EPS;
	boundTolerance                =  1.0e6 * QP_EPS;
	#endif
	...
	#ifdef __USE_SINGLE_PRECISION__
	epsFlipping                   =  5.0e1 * QP_EPS;
	#else
	epsFlipping                   =  1.0e3 * QP_EPS;
	#endif
	numRegularisationSteps        =  0;
	#ifdef __USE_SINGLE_PRECISION__
	epsRegularisation             =  2.0e1 * QP_EPS;
	numRefinementSteps            =  2;
	#else
	epsRegularisation             =  1.0e3 * QP_EPS;
	numRefinementSteps            =  1;
	#endif
	epsIterRef                    =  1.0e2 * QP_EPS;
	#ifdef __USE_SINGLE_PRECISION__
	epsLITests                    =  5.0e1 * QP_EPS;
	epsNZCTests                   =  1.0e2 * QP_EPS;
	#else
	epsLITests                    =  1.0e5 * QP_EPS;
	epsNZCTests                   =  3.0e3 * QP_EPS;
	#endif
...
returnValue Options::setToMPC( )
{
	...
	#ifdef __USE_SINGLE_PRECISION__
	terminationTolerance          =  1.0e3 * QP_EPS;
	#else
	terminationTolerance          =  1.0e9 * QP_EPS;
	#endif
...
returnValue Options::ensureConsistency( )
{
    ...
    */
	...
	{
		epsRegularisation = QP_EPS;
		needToAdjust = BT_TRUE;
	}

	if ( boundTolerance <= 0.0 )
	{
		boundTolerance = QP_EPS;
		needToAdjust = BT_TRUE;
	}

	if ( boundRelaxation <= 0.0 )
	{
		boundRelaxation = QP_EPS;
		needToAdjust = BT_TRUE;
	}

	if ( maxPrimalJump <= 0.0 )
	{
		maxPrimalJump = QP_EPS;
		needToAdjust = BT_TRUE;
	}
	...
	{
		initialFarBounds = boundRelaxation+QP_EPS;
		needToAdjust = BT_TRUE;
	}
