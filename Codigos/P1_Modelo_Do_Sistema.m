...
dados.geral.Tf = 18;            % [s] Tempo total da simulação
...
% Condições Iniciais
dados.geral.inicial.theta0 = 15;      % [°] Ângulo inicial
...
dados.pendulo.I = 0.0002;  % [kg·m^2] Momento de inércia da haste em relação ao centro
dados.pendulo.b = 0.00002;   % [N·m·s] Coeficiente de amortecimento viscoso no eixo de fixação
...
dados.carro.c = 5.5;          % [N·s/m] Coeficiente de amortecimento viscoso entre carro e guia
...
figure;
importados = importdata('Ensaios\dados_degrau_I200_D200_145928.csv');
t_import = importados(:,1);
x_import = importados(:,2:5);

posicao_import = x_import(:, 3);
angulo_import = x_import(:, 1);
velocidade_import = x_import(:, 4);
